from gview.models import Cat, Dog, Horse, Car, Breed
...
class CatListView(View):
    def get(self, request):
        ...
        breeds=  [i.name for i in Breed.objects.all()]
        datac={}
        for b in breeds:
            datac[b]=[]
        for c in stuff:
            datac[c.breed.name].append(c)

        cntx = {"cat_list": datac}
        #print(cntx)
        return render(request, "gview/cat_list.html", cntx)
        #return HttpResponse(datac["Persian"])
...
class CatDetailView(View):
    def get(self, request, pk_from_url):
        ...
        #return HttpResponse(obj.breed.name)
